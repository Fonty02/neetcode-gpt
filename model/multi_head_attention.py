...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        self.multi_heads=nn.ModuleList([self.SingleHeadAttention(embedding_dim,self.head_size) for i in range(num_heads)])
        self.out_proj=nn.Linear(attention_dim,attention_dim,False)
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        results=[]
        for l in self.multi_heads:
            out=l(embedded)
            results.append(out)
        output = self.out_proj(torch.concat(results, dim=-1))
